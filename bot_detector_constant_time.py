...
def identify(log_file):
    ...
    #     return 1
    #print(time_interval_list)
    interval_mode = mode(time_interval_list)
    interval_mode_occurence = time_interval_list.count(interval_mode)
    if interval_mode_occurence/len(time_interval_list) > threshold:
        return 0
    else:
        return 1
...
#log_file_list = ['bot_constant_time_log_3.log']

total_files = 0
correctly_identified = 0

for log_file in log_file_list:
    if 'bot_constant' in log_file or 'human' in log_file:
        total_files += 1
